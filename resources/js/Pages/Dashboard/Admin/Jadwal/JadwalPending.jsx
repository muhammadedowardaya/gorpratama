...
export default function JadwalPending({ tempat_lapangan, list_lapangan }) {
	...
	const submit = (e) => {
		...
		if (Array.isArray(semuaJadwal) && semuaJadwal.length > 0) {
			for (let i = 0; i < semuaJadwal.length; i++) {
				const semuaJadwalTanggal = moment(semuaJadwal[i].tanggal).format(
					"DD-MM-YYYY"
				);
				const semuaJadwalJamMulai = moment(semuaJadwal[i].jam_mulai, "HH:mm");
				const semuaJadwalJamSelesai = moment(
					semuaJadwal[i].jam_selesai,
					"HH:mm"
				);
				...
				if (
					semuaJadwalTanggal == dataTanggal &&
					((dataJamMulai >= semuaJadwalJamMulai &&
						dataJamMulai < semuaJadwalJamSelesai) ||
						(dataJamSelesai > semuaJadwalJamMulai &&
							dataJamSelesai <= semuaJadwalJamSelesai))
				) {
					ada_jadwal = true;
					break;
				}
		...
		if (ada_jadwal == false) {
			...
			if (user == "" || user == []) {
				...
				Swal.fire("Perhatian!", "Anda belum memilih nama pelanggan", "warning");
			} else if (data.lapangan_id == 0) {
				setShowLoading(false);
				Swal.fire("Perhatian!", "Anda belum memilih lapangan", "warning");
			} else if (
				...
				<div className="fixed top-0 right-0 left-0 bottom-0 flex justify-center p-4 z-20 mt-14 md:mt-0 backdrop-filter backdrop-blur-sm">
					<div className="relative overflow-y-auto md:overflow-y-hidden">
						<div className="login-box w-full md:w-[70vw]">
							<h1 className="text-gray-50 text-center mb-4">Tambah Jadwal</h1>
									...
									<div className="w-full mt-4">
										...
										<select
											...
											onChange={(e) => {
												...
												setData("lapangan_id", lapangan.id);
											}}
										>
									...
									<div className="w-full mt-4">
										...
										>
											...
											{statusTransaksiOptions.map((option, index) => (
												<option key={option.value} value={index.toString()}>
													{option.label}
												</option>
											))}
										</select>
